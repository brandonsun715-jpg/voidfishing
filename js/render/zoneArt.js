...
(function (VF) {
  ...
  /* The zones whose back layer is still a painted plate.
     ...
     same frame. */
  /* The painted back plates are gone.

     Seven of these existed — one flat shape at one distance with nothing in
     front of it and nothing behind it — and they were the reason seven zones
     read as backdrops however good the art on them was. Each came off as its
     zone got a landmark graph in js/data/zones.js, because a plate and a
     graph are two horizons competing and the one that means something loses.

     The Deepwater Trench keeps its plate: the trench WALLS are the zone's
     enclosing shape and they are drawn as one piece behind everything, which
     is a different job from a landmark and not one the graph should do.

     The per-zone FRONT layers all stay. They are the ambient half — the
     moonpath, the panes, the crystals, the contacts, the ring overhead — and
